...
def get_optimizer(string, model, lr):
...
if __name__ == "__main__":
  ...
  for string in list:
    ...
    with mlflow.start_run(experiment_id=76):
        ...
        optimizer = get_optimizer(string, model, lr = learning_rate)
        train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=batch_size,shuffle=True)
        validation_loader = torch.utils.data.DataLoader(dataset=validation_dataset, batch_size=5000, shuffle=True)
        
        
        cost_list, accuracy_list, dur_list_train, dur_list_val, accuracy_train_list, cost_test_list, train_time, test_time = train_model(model,train_loader,validation_loader, train_dataset, validation_dataset,optimizer, criterion,n_epochs)
